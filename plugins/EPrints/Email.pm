...
sub send_mail_via_sendmail
{
        ...
        my $data = build_email( %p )->as_string;

        # Work around mxroute/ZoneMTA DKIM body-hash mismatch: MIME::Lite
        # always inserts a blank line before the first MIME boundary in
        # print_body(), even with preamble('') set. Collapse the resulting
        # double blank line after the headers to a single one.
        $data =~ s/(\r?\n)\r?\n(--eprints-[^\r\n]+\r?\n)/$1$2/;
...
sub build_email
{
        ...
        # Use a plain boundary instead of MIME::Lite's "_----------=_<time><pid>"
        # default, and suppress the preamble line. mxroute's relay appears to
        # mishandle the default structure, invalidating the DKIM body hash.
        $mimemsg->attr( 'content-type.boundary' => 'eprints-' . time() . '-' . $$ );
        $mimemsg->preamble('');
        ...
        my $text = MIME::Lite->new(
                Type  => "TEXT",
                Data  => $data,
                Top => 0, # stops this part being marked top-level, which is what
                          # was adding MIME-Version/Date headers to each sub-part
        );
